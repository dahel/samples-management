...
const generateConditionsCombinations = (conditions) => {
  ...
  return current.values.reduce((a, string) => {
    return [
      ...a,
      ...combinations.map((combination) => [string, ...combination]),
    ];
  }, []);
};
...
const sampleStorages = combinations.map(
  ([age, patientDistrictId, patientCompanyId, visionDefectId], index) => {
